...
public:
	double a, b;
	ccomplex() {a = 0.0; b = 0.0;}
	ccomplex(double na, double nb) {a = na; b = nb;}
	const ccomplex operator+(const ccomplex &c) const
		{return ccomplex(a + c.a, b + c.b);}
	const ccomplex operator-(const ccomplex &c) const
		{return ccomplex(a - c.a, b - c.b);}
	const ccomplex operator*(const ccomplex &c) const
		{return ccomplex(a * c.a - b * c.b, a * c.b + b * c.a);}
...
{
public:
	vector<ccomplex> data;
	vector<ccomplex> roots;
	VI rev;
	int s, n;
	...
	void setSize(int ns)
	{
		...
		int i, j;
		rev = VI(n);
		data = vector<ccomplex> (n);
		roots = vector<ccomplex> (n+1);
		for (i = 0; i < n; i++)
			for (j = 0; j < s; j++)
				if ((i & (1 << j)) != 0)
					rev[i] += (1 << (s-j-1));
		
		roots[0] = ccomplex(1, 0);
		ccomplex mult = ccomplex(cos(2*PI/n), sin(2*PI/n));
		for (i = 1; i <= n; i++)
			roots[i] = roots[i-1] * mult;
	}

	void bitReverse(vector<ccomplex> &array)
	{
		vector<ccomplex> temp(n);
		int i;
		for (i = 0; i < n; i++)
			temp[i] = array[rev[i]];
		for (i = 0; i < n; i++)
	...
	static VI convolution(VI &a, VI &b)
	{
		...
		for (i = 0; i < alen; i++) pga.data[i] = ccomplex(a[i], 0);
		for (i = alen; i < n; i++)	pga.data[i] = ccomplex(0, 0);
		pga.transform();

		pgb.setSize(s);	// fill and transform second array
		for (i = 0; i < blen; i++)	pgb.data[i] = ccomplex(b[i], 0);
		for (i = blen; i < n; i++)	pgb.data[i] = ccomplex(0, 0);
		pgb.transform();
